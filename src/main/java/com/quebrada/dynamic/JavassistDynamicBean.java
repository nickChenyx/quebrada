...
public class JavassistDynamicBean implements DynamicBean {
		...
		try {
			...
			try {
				...
				if (beanType.equals(HtmlBean)) {
					CtClass htmlBeanInterface = pool.get("com.quebrada.spider.HtmlBean");
					clazz.addInterface(htmlBeanInterface);
				} else {
					CtClass jsonBeanInterface = pool.get("com.quebrada.spider.JsonBean");
	...
	@Override
	public JavassistDynamicBean quebrada(String matchUrl, String... pipelines) {
		quebrada(new String[]{matchUrl}, pipelines);
		return this;
	}

	@Override
	public JavassistDynamicBean quebrada(String matchUrl, String downloader, int timeout, String... pipelines) {
		quebrada(new String[]{matchUrl}, "", 3000, pipelines);
		return this;
	}

	@Override
	public JavassistDynamicBean quebrada(String[] matchUrl, String... pipelines) {
		quebrada(matchUrl, "", 3000, pipelines);
		return this;
	}
	...
	public JavassistDynamicBean quebrada(String[] matchUrl, String downloader, int timeout, String... pipelines) {
		...
		Annotation annot = new Annotation(Quebrada.class.getName(), cpool);
		// matchUrl
	...
	public Class<?> loadClass() {
		try {
			Class<?> loadClass = clazz.toClass(QuebradaClassLoader.get(), null);
	...
	public Class<?> register() {
		...
		if (loadClass.getAnnotation(Quebrada.class) != null) {
			QuebradaClassLoader.get().addClass(loadClass.getName(), loadClass);
		}
